...
where $\theta$ is a shape parameter, set to 0.9999, and A~m~ is the hyperbolic minimum of A~c~ and A~j~. 
...
To demonstrate a practical application of the key functions in the package, I use data from Medlyn et al. [-@medlyn2007] on *Eucalyptus delegatensis*. Both A-C~i~ curves and 'spot gas exchange' data (i.e. leaf gas exchange measurements at prevailing environmental conditions) were collected. Using the `fitacis` function, it is straightforward to fit XXX curves to the measured A-C~i~ curves in one line of code, and make standard plots of the fitted curves (shown iin Fig. 5a one option). The fitted coefficients are easily extracted, and used to plot a standard comparison of fitted Vcmax and Jmax values (Fig. 5b), which show the typical correlation between the two. Next I fit the spot gas exchange data, yielding estimates of g1 and g0, and we can quickly compare modelled to measured g~s~ (Fig. 5c), and compare the model prediction of instantaneous transpiration efficiency (A/E) to measurements along the variation in D (Fig. 5d). This simple example application is available in the published repository (see Methods).
