...
Remko Duursma
E: remkoduursma@westernsydney.edu.au
T: +61(0)245701806
...
\
We do not repeat a detailed description of the FvCB model here, as it has been described many times [e.g. @bernacchi2001; @medlyn2002]. But generally it is of the form,
...
\
In the practical application of the FVcB model, it is often assumed that C~c~ is equal to the intercellular [CO~2~] concentration (C~i~), which is useful because C~i~ can be estimated from leaf gas exchange data [@sharkey1982], and this avoids the need to estimate the conductance from intercellular airspaces to the chloroplast (mesophyll conductance, g~m~). In the remainder of this article I will use C~c~ as the driver of photosynthesis, but point out that this can be replaced by C~i~ if the user does not have an estimate of g~m~. When g~m~ is known, C~c~ is calculated from C~i~ with Eq. 2. 
...
The main tools included in the plantecophys package are to a) fit A-C~i~ curves to estimate V~cmax~, J~max~ and R~d~, b) fit Ball-Berry type models, c) simulate from the coupled leaf gas exchange model and d) calculate the optimal stomatal conductance. A list of the key functions in the package are summarized in Table 1. 
...
The 'plantecophys' package is implemented in R, has no dependencies on other packages, and does not require compilation (i.e. it is written in native R only). As such it builds easily, and is highly portable. A stable release is available on CRAN. The source code is maintained with git version control, and is hosted in an online repository (http://www.bitbucket.org/remkoduursma/plantecophys). The repository includes an issue tracker, where users can suggest changes or report bugs.  This paper describes version `r packageVersion("plantecophys")` (git SHA `r cur_sha`).

\
All code used in this article (including the code to generate the actual article in markdown), can be downloaded from http://www.github.com/duursmaplantecophys2015.
...
\
**Figure 2**. Standard output from the `fitaci` function. A~n~ is the net photosynthetic rate, C~i~ the intercellular CO~2~ concentration. Symbols are measurements, the black line the fitted FvCB model of photosynthesis. Colored lines indicate the two photosynthesis rates in the FvCB model. In the default mode, the `fitaci` function estimates V~cmax~, J~max~ and R~d~ from the fitted curve. Optionally, R~d~ is provided as an input, for example when it was measured separately. In this example, V~cmax~ was estimated as `r round(coef(f)[1],1)`, J~max~ was `r round(coef(f)[2],1)` and R~d~ was `r round(coef(f)[3],1)` (all in units of $\mu$mol m^-2^ s^-1^).
...
To demonstrate a practical application of the key functions in the package, I use field-collected data from Medlyn et al. [-@medlyn2007] on *Eucalyptus delegatensis*. Both A-C~i~ curves and 'spot gas exchange' data (i.e. leaf gas exchange measurements at prevailing environmental conditions) were collected. Using the `fitacis` function, it is straightforward to fit all `r length(acifits)` curves to the A-C~i~ data, and make standard plots of the fitted curves (shown in Fig. 5a). The fitted coefficients are easily extracted, and used to plot a standard comparison of fitted V~cmax~ and J~max~ values (Fig. 5b), which show the typical correlation between the two. Next I fit the spot gas exchange data, yielding estimates of g~1~ and (optionally) g~0~, and we can quickly compare modelled to measured g~s~ (Fig. 5c), and compare the model prediction of instantaneous transpiration efficiency (A~n~/E) to measurements along the variation in D (Fig. 5d). This simple example application is available in the published repository (see Methods). The simplified code for this example (panels a-c), omitting special formatting and a few minor settings is given below.

```
# Fit A-Ci curves. In this case, each separate curve is indexed by a column names 'Curve',
...
# Data are already read into a dataframe (tumspot), and have standard names (or they can be set).
