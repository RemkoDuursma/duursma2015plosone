...
\
In the practical application of the FvCB model, when leaf gas exchange is measured with a portable gas exchange instrument, estimates of C~c~ are difficult to obtain because they require an estimate of the mesophyll conductance. In this case, it is customary to us the intercellular [CO~2~] concentration (C~i~) as the driver of photosynthesis. This approach is useful because C~i~ can be estimated from concurrent measurements of CO~2~ and H~2~O flux [@sharkey1982]. In the remainder of this article I will use C~c~ as the driver of photosynthesis, but point out that this can be replaced by C~i~ if the user does not have an estimate of g~m~. When this is is known, C~c~ is calculated from C~i~ with Eq. 2. 
